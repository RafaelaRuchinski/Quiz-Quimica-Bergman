...
local function quizGoodAlternative(event)
	...
	opcCerta.x = event.target.x + event.target.width - 100
...
local function quizBadAlternative(event)
	...
	opcErrada.x = event.target.x + 100
	opcErrada.y = event.target.y

	local opcCerta = display.newImageRect(questionGroup, "images/certo.png", 66, 66)
	opcCerta.x = correctX + 100
	opcCerta.y = correctY

	if composer.getVariable("lives") == 0 then
	  composer.gotoScene("gameover", { time=2000, effect="crossFade" })
  else
		-- Aguarda 2 segundos
		composer.gotoScene("game", { time=2000, effect="crossFade" })
	end
